import { afterEach, describe, expect, test, vi } from "vitest";
import { MyInfoResponse } from "api";
import { when } from "mobx";
import { DeckListStore } from "./deck-list-store.ts";

vi.mock("mobx-persist-store", () => {
  return {
    makePersistable: () => {},
  };
});

const userStoreMock = vi.hoisted(() => ({
  myId: 111,
  setUser: () => {},
  isPaid: true,
}));

vi.mock("./user-store.ts", () => {
  return {
    userStore: userStoreMock,
  };
});

vi.mock("../platform/storage-adapter.ts", () => {
  return {
    storageAdapter: {},
  };
});

const notifyPaymentSuccessMock = vi.hoisted(() => vi.fn());
const notifyPaymentFailedMock = vi.hoisted(() => vi.fn());

vi.mock("../screens/shared/notify-payment.ts", () => {
  return {
    notifyPaymentFailed: notifyPaymentFailedMock,
    notifyPaymentSuccess: notifyPaymentSuccessMock,
  };
});

vi.mock("../api/api.ts", () => {
  return {
    getSharedDeckRequest: () => {},
  };
});

vi.mock("../api/trpc-api.ts", () => {
  return {
    api: {
      deck: {
        deckWithCards: { query: () => {} },
        addToMine: { mutate: () => {} },
      },
      folder: {
        folderWithDecksCards: { query: () => {} },
      },
      me: {
        info: {
          query: (): Promise<MyInfoResponse> => {
            return Promise.resolve({
              user: {
                id: 111,
                firstName: "Test",
                isRemindEnabled: true,
                languageCode: "uk",
                lastName: "Testov",
                lastRemindedDate: null,
                isSpeakingCardEnabled: false,
                isAdmin: false,
                username: "test",
              },
              cardsToReview: [
                { id: 1, type: "new", deckId: 1 },
                { id: 2, type: "repeat", deckId: 1 },
                { id: 3, type: "new", deckId: 1 },
                { id: 4, type: "new", deckId: 2 },
                { id: 5, type: "repeat", deckId: 1 },
              ],
              plans: [],
              myDecks: [
                {
                  id: 1,
                  createdAt: "",
                  availableIn: null,
                  name: "Deck 1",
                  isPublic: false,
                  shareId: "1",
                  authorId: 1,
                  description: "",
                  speakLocale: null,
                  speakField: null,
                  categoryId: null,
                  deckCategory: null,
                  cardInputModeId: null,
                  deckCards: [
                    {
                      id: 1,
                      createdAt: "",
                      deckId: 1,
                      front: "d1c1 - f",
                      back: "d1c1 - b",
                      answerType: "remember",
                      example: null,
                      answers: null,
                      options: null,
                    },
                    {
                      id: 2,
                      createdAt: "",
                      deckId: 1,
                      front: "d1c2 - f",
                      back: "d1c2 - b",
                      answerType: "remember",
                      example: null,
                      answers: null,
                      options: null,
                    },
                    {
                      id: 3,
                      createdAt: "",
                      deckId: 1,
                      front: "d1c3 - f",
                      back: "d1c3 - b",
                      answerType: "remember",
                      example: null,
                      answers: null,
                      options: null,
                    },
                    {
                      id: 5,
                      createdAt: "",
                      deckId: 1,
                      front: "d1c5 - f",
                      back: "d1c5 - b",
                      answerType: "remember",
                      example: null,
                      answers: null,
                      options: null,
                    },
                  ],
                },
                {
                  id: 2,
                  createdAt: "",
                  availableIn: null,
                  name: "Deck 2",
                  isPublic: false,
                  shareId: "2",
                  authorId: 1,
                  description: "",
                  speakLocale: null,
                  speakField: null,
                  deckCategory: null,
                  categoryId: null,
                  cardInputModeId: null,
                  deckCards: [
                    {
                      id: 4,
                      createdAt: "",
                      deckId: 2,
                      front: "d2c1 - f",
                      back: "d2c1 - b",
                      answerType: "remember",
                      example: null,
                      answers: null,
                      options: null,
                    },
                  ],
                },
              ],
              publicDecks: [],
              folders: [],
            });
          },
        },
      },
    },
  };
});

vi.mock("./screen-store", () => {
  return {
    screenStore: {
      screen: {
        type: "deckMine",
        deckId: 1,
      },
    },
  };
});

vi.mock("../lib/platform/telegram/haptics.ts", () => {
  return {
    hapticNotification: () => {},
    hapticImpact: () => {},
  };
});

vi.mock("../lib/platform/show-confirm.ts", () => {
  return {
    showConfirm: () => {
      return Promise.resolve(true);
    },
  };
});

vi.mock("../translations/t.ts", () => {
  return {
    t: (arg: any) => arg,
  };
});

vi.mock("../screens/shared/snackbar/snackbar.tsx", () => {
  return {
    notifySuccess: () => {},
    notifyError: () => {},
  };
});

describe("deck list store", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  test("test load decks", async () => {
    const store = new DeckListStore();
    store.load();

    await when(() => !!store.myInfo);

    expect(store.publicDecks).toHaveLength(0);
    expect(store.newCardsCount).toBe(3);
    expect(store.selectedDeck?.cardsToReview).toMatchInlineSnapshot(`
      [
        {
          "answerType": "remember",
          "answers": null,
          "back": "d1c5 - b",
          "createdAt": "",
          "deckId": 1,
          "example": null,
          "front": "d1c5 - f",
          "id": 5,
          "options": null,
          "type": "repeat",
        },
        {
          "answerType": "remember",
          "answers": null,
          "back": "d1c2 - b",
          "createdAt": "",
          "deckId": 1,
          "example": null,
          "front": "d1c2 - f",
          "id": 2,
          "options": null,
          "type": "repeat",
        },
        {
          "answerType": "remember",
          "answers": null,
          "back": "d1c1 - b",
          "createdAt": "",
          "deckId": 1,
          "example": null,
          "front": "d1c1 - f",
          "id": 1,
          "options": null,
          "type": "new",
        },
        {
          "answerType": "remember",
          "answers": null,
          "back": "d1c3 - b",
          "createdAt": "",
          "deckId": 1,
          "example": null,
          "front": "d1c3 - f",
          "id": 3,
          "options": null,
          "type": "new",
        },
      ]
    `);
  });
});
